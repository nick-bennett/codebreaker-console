...
public class Player {
  ...
  private static final String INVALID_CHARS_FORMAT_KEY = "invalid_chars_format";
  ...
  public void play(String pool, int length) throws IOException {
    ...
    Guess guess;
    String validGuessPattern =
        String.format(GUESS_VALIDATION_FORMAT, game.getPool(), game.getLength());
    do {
  ...
  private String getGuess(Game game, String validationPattern) {
    ...
    do {
      ...
      if (input.length() != game.getLength()) { // Length is invalid.
        System.out.printf(bundle.getString(INVALID_LENGTH_FORMAT_KEY), input, game.getLength());
      } else if (!input.matches(validationPattern)) { // Guess contains invalid characters.
        System.out.printf(bundle.getString(INVALID_CHARS_FORMAT_KEY), input, game.getPool());
      } else { // Guess is valid.
        break;
      }
    } while (true);
